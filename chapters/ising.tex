...
以下结果中，Ising 点阵大小分别为 $8 \times 8$、$16 \times 16$、$32 \times 32$、$64 \times 64$ 和
$128 \times 128$
\footnote{在 $8 \times 8$ 点阵中各物理量涨落较大，因此在部分图中没有绘出。}，
并采用周期性边界条件。选取温度区间 $T/J \in [1,\,3.6]$，外磁场为零。进行
\num{900} MCS 之后，认为体系达到平衡态。之后再进行 \num{100} MCS，并在其中取系综平均计算各物理量。
每个温度下，重复模拟 50 次，统计各物理量的平均值及标准差（作为不确定度）。模拟结果见
图~\ref{fig:ising-e-m} 和。

\begin{figure}[htb]
  \centering
  \begin{subfigure}[b]{0.49\textwidth}
    \centering
    \tikzinput{ising-energy}
    \caption{能量}
  \end{subfigure}
  \begin{subfigure}[b]{0.49\textwidth}
    \centering
    \tikzinput{ising-magnet}
    \caption{磁化强度}
  \end{subfigure}
...
\begin{figure}
  \begin{subfigure}[b]{0.49\textwidth}
    \tikzinput{ising-cv-fit-i}
    \caption{临界温度随点阵尺度的变化。注意 $x$ 轴、$y$ 轴均取了对数坐标}
  \end{subfigure}
  \begin{subfigure}[b]{0.49\textwidth}
    \tikzinput{ising-cv-fit-ii}
