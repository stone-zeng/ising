...
从图中可以看出，线性分类模型大致可以区分无序相有有序相，但效果不好，误差较大。理论上，
$P \sim tL^{1/\nu}$ 曲线应当完全重合，但图~\ref{fig:ising-learning-linear-scaled} 显示出了与理论结果相当大的偏离。对于较大点阵，很明显可以看到，线性模型几乎不能分清有序项的存在。这其实是训练部充分的体现。
...
接下来我们利用如图~\ref{fig:neural-net} 所示的多层神经网络来对 Ising 模型的相进行分类。所用数据与 \ref{subsec:ising-linear-train-result}~小节中相同。具体训练参数如下：
...
与线性模型给出的结果图~\ref{fig:ising-learning-linear} 相比，神经网络的效果要好很多，尽管我们只是增加了一层网络。除了 $8 \times 8$ 点阵仍然存在一定误差，其他尺寸下神经网络均可清晰地分辨不同相的存在。图~\ref{fig:ising-learning-net-scaled} 也显示出了做尺度缩放之后的一致性。

利用概率分布曲线，我们可以插值并反解出临界温度的位置（对应概率 $P=0.5$）。关于 $1/L$ 拟合，得到图~\ref{fig:ising-learning-net-fit}。给出的拟合结果为 $T_*/J=\num{2.2488(11)}$，这与理论值相差不大。
